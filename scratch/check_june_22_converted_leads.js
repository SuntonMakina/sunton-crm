...
async function run() {
  ...
  const { data: leads, error } = await supabase
    .from('leads')
    .select('*, lead_sources(name), communication_channels(name)')
  ...
  const june22Converted = leads.filter(l => {
    const d = l.first_contact_date || l.first_contact_at || l.created_at;
    return getLocalDateStringWithShift(d) === '2026-06-22';
  });
  
  console.log(`Converted leads on June 22: ${june22Converted.length}`);
  june22Converted.forEach(l => {
    console.log({
      id: l.id,
