...
async function deleteLeadCascade(token, firstName, lastName, phone) {
  console.log(`Checking for lead: ${firstName} ${lastName} with phone ${phone}`);
  
  // Find leads matching name and phone
  const uniqueLeads = await callSupabase('leads', token, `first_name=eq.${encodeURIComponent(firstName)}&last_name=eq.${encodeURIComponent(lastName)}&phone=eq.${encodeURIComponent(phone)}`);

  if (uniqueLeads.length === 0) {
    console.log(`No lead found in DB matching ${firstName} ${lastName} (${phone})`);
    return;
  }
...
async function run() {
  try {
    ...
    await deleteLeadCascade(token, 'Seda', 'Kara', '05328276314'); // L-2696
    await deleteLeadCascade(token, 'Seda', 'Kara', '05352648331'); // L-2697
